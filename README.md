...
> 카카오 응답 파싱은 개발자 문서를 보고 쓴 것이고 **실물 응답으로 확인하지
> 못했다** (개발 컨테이너에서 `dapi.kakao.com` 이 막혀 있다).
> 로컬에서 `python scripts/smoke_geocode.py "강남역 스타벅스"` 를 한 번 돌려
> 확인할 것. 필드명이 다르면 그 스크립트가 저장한 픽스처를 보고 고친다.
...
5. **카카오 파싱은 미검증** — 개발 컨테이너에서 `dapi.kakao.com` 이 막혀 실물
   응답을 못 받았다. 장소 검색이 이상하면 먼저 `scripts/smoke_geocode.py` 를
   돌려볼 것. 키가 없으면 정류장 이름만 검색되는 게 정상이다.
